...
public class SkinApplierBukkit implements SkinApplierAccess<Player> {
    ...
    private void normalOtherRefresh(Player player) {
        ...
        // Refresh other players in the joining player's view to ensure bidirectional skin sync
        for (Player otherPlayer : getOnlinePlayers()) {
            if (!otherPlayer.getUniqueId().equals(player.getUniqueId()) && player.canSee(otherPlayer)) {
                hideAndShow(player, otherPlayer);
            }
        }
    }
    ...
    private void teleportOtherRefresh(Player player) {
        ...
        for (Player otherPlayer : getOnlinePlayers()) {
            if (!otherPlayer.getUniqueId().equals(player.getUniqueId()) && player.canSee(otherPlayer)) {
                refresh.resendInfoPackets(otherPlayer, player);
            }
        }
